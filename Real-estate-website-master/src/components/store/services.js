...
export const archiveService = createAsyncThunk("archiveService", async (id) => {
  await axios.put(`http://localhost:7000/services/${id}/archive`);
  return id;
});
...
const servicesSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(archiveService.fulfilled, (state, action) => {
      // Mettez à jour le service dans la liste pour marquer comme archivé
      state.items = state.items.map(service =>
        service.id === action.payload ? { ...service, archived: true } : service
      );
    });

...
export const fetchClient = createAsyncThunk("client/fetchClient", async (id) => {
...
export const updateClient = createAsyncThunk("client/updateClient", async ({ id, body }) => {
...
const clientSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(updateClient.fulfilled, (state, action) => {
      ...
      const index = state.clients.items.findIndex(client => client.id === updatedClient.id);
      if (index !== -1) {
    ...
    builder.addCase(deleteClient.fulfilled, (state, action) => {
      state.clients.items = state.clients.items.filter(client => client.id !== action.payload);
      state.clients.count--;
    });
  },
});

...
export const sendUser = createAsyncThunk("user/sendUser", async (body) => {
  const response = await axios.post("http://localhost:7000/user", body);
  return response.data;
});

export const updateUser = createAsyncThunk("user/updateUser", async ({ id, body }) => {
  try {
    const response = await axios.patch(`http://localhost:7000/user/${id}`, body);
    console.log("Réponse du backend après la mise à jour :", response.data);
    return response.data;
  } catch (error) {
    console.error('Erreur lors de la mise à jour de user sur le backend :', error);
    throw error;
  }
});
